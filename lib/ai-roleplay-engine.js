...
class AIRoleplayEngine {
  ...
  async generateResponse(sessionId, userMessage) {
    try {
      ...
      // Create AI chat instance with scenario context
      ...
      const userMsg = new UserMessage(fullPrompt);
      const response = await chat.send_message(userMsg);
...
For each learning objective, determine if the student demonstrated that skill in their message. Respond with only a JSON object:
{
  "achievements": [
    ...
    {"objective_index": 1, "achieved": true/false, "evidence": "brief explanation"}
      ...
      const analysisChat = new LlmChat(
        this.apiKey,
      ...
      const analysisMsg = new UserMessage(analysisPrompt);
      const analysisResult = await analysisChat.send_message(analysisMsg);
